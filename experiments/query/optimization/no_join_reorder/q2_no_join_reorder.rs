...
fn main() -> Result<()> {
    ...
    tracing::info!("join order: partsupp -> part -> supplier -> nation -> region");

    let ps_p_table = part_table.inner_join(
        "p_partkey",
        "ps_partkey", 
        &partsupp_table,
        &mut mpc_exec_args,
    ...
    let ps_p_s_table = supplier_table.inner_join(
        "s_suppkey",
        "ps_suppkey",
        &ps_p_table,
        &mut mpc_exec_args,
    ...
    let ps_p_s_n_table = nation_table.inner_join(
        "n_nationkey",
        "s_nationkey",
        &ps_p_s_table,
        &mut mpc_exec_args,
    ...
    let sub_table = region_table.inner_join(
        "r_regionkey",
        "n_regionkey",
        &ps_p_s_n_table,
        &mut mpc_exec_args,
    ...
    tracing::info!("Q2 no-join-reorder execution completed");

    if party_id == PartyID::ID0 {
        tracing::info!("Total Q2 no-join-reorder execution time: {:?}", tot_start.elapsed());
    }
    print_communication_stats(&mpc_exec_args, "Q2 no-join-reorder");
