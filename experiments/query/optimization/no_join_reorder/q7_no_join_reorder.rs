...
fn main() -> Result<()> {
    ...
    tracing::info!("join : supplier, lineitem, orders, customer, nation n1, nation n2");
    ...
    let s_l_o_table = orders_table.inner_join(
        "o_orderkey",
        "l_orderkey",
        &s_l_table,
        &mut mpc_exec_args,
    ...
    let mut s_l_o_c_table = customer_table.inner_join(
        "c_custkey",
        "o_custkey",
        &s_l_o_table,
        &mut mpc_exec_args,
    )?;
    s_l_o_c_table.delete_column("o_custkey");

    let mut s_l_o_c_n1_table = nation1_table.inner_join(
        "n1_nationkey",
        "s_nationkey",
        &s_l_o_c_table,
        &mut mpc_exec_args,
    )?;
    s_l_o_c_n1_table.delete_column("s_nationkey");

    let mut final_table = nation2_table.inner_join(
        "n2_nationkey",
        "c_nationkey",
        &s_l_o_c_n1_table,
        &mut mpc_exec_args,
    )?;
    final_table.delete_column("c_nationkey");
