...
fn main() -> Result<()> {
    ...
    */
    ...
    tracing::info!("Q5 no-secure-cut execution completed");

    if party_id == PartyID::ID0 {
        tracing::info!("Total Q5 no-secure-cut execution time: {:?}", tot_start.elapsed());
    }
    print_communication_stats(&mpc_exec_args, "Q5 no-secure-cut");
