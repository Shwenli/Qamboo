...
fn main() -> Result<()> {
    ...
    */
    ...
    //这里好像可以只按照c_custkey分组，因为其他字段和c_custkey是一一对应的
...
//************* polars verification *************//
    ...
    if state0.id == PartyID::ID0 {
        ...
        // Check top 10
        //let len = std::cmp::min(10, polars_o_custkey.len());
        //eprintln!("Multiparty computation result: ");
        //eprintln!("mpc_o_custkey: {:?}", &mpc_o_custkey[0..len]);
        //eprintln!("mpc_revenue: {:?}", &mpc_revenue[0..len]);
        //eprintln!("Polars result: ");
        //eprintln!("polars_o_custkey: {:?}", &polars_o_custkey[0..len]);
        //eprintln!("polars_revenue: {:?}", &polars_revenue[0..len]);
