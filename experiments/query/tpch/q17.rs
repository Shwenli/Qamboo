...
fn main() -> Result<()> {
    ...
    let p_container_binary = tpch_database_gen::convert_binary_from_arithmetic(
        &part_table["p_container"],
        &mut mpc_exec_args,
    )?;
    ...
    */
    ...
    let avg_yearly = div_share_by_public(sum_l_extendedprice, &RingElement(7u64), &net0, mpc_exec_args.state0)?;

    let open_avg_yearly = open(avg_yearly, &net0)?;
...
/*
* 物理计划执行顺序 (Physical Plan):
* 1. 扫描 LineItem 和 Part 表 (Tables Generation)。
* 2. 数据转换: 将 p_brand 和 p_container 转为二进制分享 (Data Conversion)。
* 3. 投影: 筛选 LineItem 和 Part 表的必要列 (Projection)。
* 4. 过滤: 在 Part 表上对 p_brand 和 p_container 进行相等性过滤 (Filter Part)。
* 5. 连接: Part 表与 LineItem 表基于 PartKey 进行内连接 (Join -> part_lineitem_table)。
* 6. 子查询分组: 对 part_lineitem_table 按 l_partkey 分组 (Group By l_partkey)。
* 7. 子查询聚合: 计算 sum(l_quantity) 和 count(l_quantity) (Aggregation)。
* 8. 子查询计算: 计算阈值 0.2 * avg(l_quantity)，代码中体现为除以 5 (Calculation)。
* 9. 截断: 将子查询结果截断为 Part 表大小 (Limit/Cut)。
* 10. 连接: 将子查询结果与 part_lineitem_table 再次连接 (Join with sub-table)。
* 11. 过滤: 筛选 l_quantity < avg_l_quantity (Filter LessThan)。
* 12. 最终聚合: 计算 sum(l_extendedprice) (Final Aggregation)。
* 13. 最终计算: 结果除以 7.0 得到 avg_yearly (Final Calculation)。
*/

/*
* Physical Plan Execution Order:
* 1. Tables Generation: Scan LineItem and Part tables.
* 2. Data Conversion: Convert p_brand and p_container to binary shares.
* 3. Projection: Select necessary columns from LineItem and Part tables.
* 4. Filter Part: Apply equality filter on Part table for p_brand and p_container.
* 5. Join: Inner join Part table with LineItem table on PartKey (producing part_lineitem_table).
* 6. Group By: Group part_lineitem_table by l_partkey (for subquery).
* 7. Aggregation: Compute sum(l_quantity) and count(l_quantity).
* 8. Calculation: Calculate threshold 0.2 * avg(l_quantity) (dividing by 5).
* 9. Limit/Cut: Truncate subquery result to Part table size.
* 10. Join with Sub-table: Join calculated threshold back with part_lineitem_table.
* 11. Filter LessThan: Filter rows where l_quantity < avg_l_quantity.
* 12. Final Aggregation: Compute sum(l_extendedprice).
* 13. Final Calculation: Divide result by 7.0 to get avg_yearly.
*/
