...
/*
 * ORQ-style Q6 with an arithmetic valid column (ablation variant of q6_orq /
 * q6_orq_bit). The predicate evaluation is identical to q6_orq — subtractions
 * are moved to preprocessing and every predicate is a local sign extraction —
 * but the valid column is an *arithmetic* sharing, like in the other TPC-H
 * queries. Folding the predicate into valid therefore goes through the generic
 * filter_directed_by_bool path: a2b(valid) -> AND with the composed predicate
 * -> b2a back to arithmetic. The revenue masking then needs no bit_inject
 * since valid is already arithmetic.
 */
...
use experiments::gen_valid_column_u64_ring;
...
fn main() -> Result<()> {
    ...
    // ablation: valid column is an arithmetic sharing, like the other TPC-H queries
    // (replaces the binary valid column inserted by gen_lineitem_table)
    let valid = gen_valid_column_u64_ring(lineitem_table.num_rows(), "valid".to_string(), ShareType::Arithmetic, mpc_exec_args.nets, party_id);
    ...
    print_communication_stats(&mpc_exec_args, "Q6_ORQ_ARI_VALID preprocessing");
    ...
    tracing::info!("Q6_ORQ_ARI_VALID start");
    ...
    // arithmetic valid: a2b(valid) -> AND with the predicate -> b2a, the same
    // filter path the other TPC-H queries use
    ...
    // valid is already arithmetic: mask the revenue directly, no bit_inject needed
    ...
    tracing::info!("Q6_ORQ_ARI_VALID execution completed");

    if party_id == PartyID::ID0 {
        tracing::info!("Total Q6_ORQ_ARI_VALID execution time: {:?}", tot_start.elapsed());
    }
    print_communication_stats(&mpc_exec_args, "Q6_ORQ_ARI_VALID total (preprocessing + query)");
...
//************* polars verification *************//
    ...
    if party_id == PartyID::ID0 {
        ...
        tracing::info!("Q6_ORQ_ARI_VALID: MPC result matches polars result !");
    }
