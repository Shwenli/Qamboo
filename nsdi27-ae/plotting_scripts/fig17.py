...
def plot_primitives(csv_file, output_file):
    # Read CSV data
    df = pd.read_csv(csv_file)

    # Separate multiplication and less-than data
    df_mul = df[df['primitive'] == 'mul'].copy()
    df_lt = df[df['primitive'] == 'lt'].copy()
    ...
    x_labels = [f"$2^{{{x}}}$" if (x - 16) % 2 == 0 else '' for x in range(16, 26)]
    ...
    # Plot the multiplication subplot
    plot_subplot(ax1, df_mul, 'Multiplication')

    # Plot the less-than subplot
    plot_subplot(ax2, df_lt, 'Less-than')

    # Shared x and y axis labels, centered
    fig.text(0.5, 0.05, 'Number of Elements', ha='center', fontsize=7, fontweight='bold')
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description='Plot basic primitives (multiplication, less-than) vs. MP-SPDZ from CSV')
    ...
    csv_path = os.path.join(script_dir, '..', 'data', args.source, 'fig17.csv')
    output_path = os.path.join(script_dir, '..', 'figures', args.source, 'fig17.pdf')

    # Ensure the output directory exists
    os.makedirs(os.path.dirname(output_path), exist_ok=True)

    plot_primitives(csv_path, output_path)
