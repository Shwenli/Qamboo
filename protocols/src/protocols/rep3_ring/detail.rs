...
/// Qamboo add : Adds a constant to a share for div .
pub fn low_depth_binary_add_const<T: IntRing2k, N: Network>(
    x1: &Rep3RingShare<T>,
    x2: &RingElement<T>,
    net: &N,
    state: &mut Rep3State,
    ...
    Standard: Distribution<T>,
{
    ...
    let p = binary::xor_public(x1, x2, state.id);
    let g = x1 & x2;
...
/// Adds two vectors of ring elements
pub fn low_depth_binary_add<T: IntRing2k, N: Network>(
    x1: &Rep3RingShare<T>,
    x2: &Rep3RingShare<T>,
    net: &N,
    state: &mut Rep3State,
...
pub fn low_depth_binary_sub_with_carry<T: IntRing2k, N: Network>(
    x1: &Rep3RingShare<T>,
    x2: &Rep3RingShare<T>,
    net: &N,
    state: &mut Rep3State,
...
/// Calculates 2^k + x1 - x2
pub fn low_depth_binary_sub_by_const_with_carry<T: IntRing2k, N: Network>(
    x1: &Rep3RingShare<T>,
    x2: &RingElement<T>,
    net: &N,
    state: &mut Rep3State,
...
/// Calculates 2^k + x1 - x2
pub fn low_depth_binary_sub_from_const_with_carry<T: IntRing2k, N: Network>(
    x1: &RingElement<T>,
    x2: &Rep3RingShare<T>,
    net: &N,
    state: &mut Rep3State,
...
pub fn unsigned_ge<T: IntRing2k, N: Network>(
    x: Rep3RingShare<T>,
    y: Rep3RingShare<T>,
    net: &N,
    state: &mut Rep3State,
...
where
    Standard: Distribution<T>,
{
    let a_bits = conversion::a2b(x, net, state)?;
    let b_bits = conversion::a2b(y, net, state)?;
...
pub fn unsigned_ge_const_lhs<T: IntRing2k, N: Network>(
    x: RingElement<T>,
    y: Rep3RingShare<T>,
    net: &N,
    state: &mut Rep3State,
...
pub fn unsigned_ge_const_rhs<T: IntRing2k, N: Network>(
    x: Rep3RingShare<T>,
    y: RingElement<T>,
    net: &N,
    state: &mut Rep3State,
