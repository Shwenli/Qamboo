#!/bin/bash
#! run with: ./run_aspirin_ssh_rdma.sh 4 0.01
...
BIN_PATH="smc_run ./target/release/aspirin"
...
RUSTFLAGS="-C target-cpu=native" cargo build --release --package experiments --bin aspirin --features tcp
scp target/release/aspirin $HOST1:$PROJECT_PATH/target/release/
scp target/release/aspirin $HOST2:$PROJECT_PATH/target/release/
