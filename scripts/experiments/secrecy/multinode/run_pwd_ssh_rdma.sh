#!/bin/bash
#! run with: ./run_pwd_ssh_rdma.sh 4 0.01
...
BIN_PATH="smc_run ./target/release/pwd"
...
scp target/release/pwd $HOST1:$PROJECT_PATH/target/release/
scp target/release/pwd $HOST2:$PROJECT_PATH/target/release/
