...
# Multi-thread Radix Sort (Multinode)
# Usage: ./run_radix_multi_ssh.sh [NUM_COMMTHREADS] [SHIFT] [HOSTS]
...
for h in "${HOSTS_ARR[@]}"; do
    is_local "$h" || scp target/release/radix_sort_multi "$h:$PROJECT_PATH/target/release/"
done

# Party i runs on the i-th host (with RAYON_NUM_THREADS everywhere)
export RAYON_NUM_THREADS=$RAYON_T
for p in 0 1 2; do
    h="${HOSTS_ARR[$p]}"
    if is_local "$h"; then
        RAYON_NUM_THREADS=$RAYON_T $BIN_PATH -c experiments/net/multinode/ -p $p -t $NUM_COMMTHREADS -s $SHIFT &
    else
        ssh "$h" "cd $PROJECT_PATH && export RAYON_NUM_THREADS=$RAYON_T && $BIN_PATH -c experiments/net/multinode/ -p $p -t $NUM_COMMTHREADS -s $SHIFT" &
    fi
done
