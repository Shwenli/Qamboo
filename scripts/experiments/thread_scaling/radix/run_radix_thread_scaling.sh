...
# Usage: ./run_radix_thread_scaling.sh [SHIFT] [-h HOSTS]
#
# Tests:
...
# Parse arguments
if [ "$#" -lt 1 ]; then
    echo "Usage: $0 <SHIFT> [-h HOSTS]"
    echo "  <SHIFT>       : 2^SHIFT rows (default: 20, i.e., 1M rows)"
    echo "  -h HOSTS      : Comma-separated host per party (default: node0,node1,node2)"
    exit 1
fi

shift 1

# Parse optional -h argument
while [[ $# -gt 0 ]]; do
    case $1 in
        -h)
            HOSTS="$2"
...
if [ -f "$SCRIPT_PATH" ]; then
    ...
    (cd "$MULTINODE_DIR" && ./run_radix_single_ssh.sh "$SHIFT" "$HOSTS") 2>&1 | tee >(grep --line-buffered "INFO Total" >> "$LOG_FILE")
    if [ $? -eq 0 ]; then
...
for RAYON_T in "${RAYON_THREADS_LIST[@]}"; do
    for COMM_T in "${COMM_THREADS_LIST[@]}"; do
        ...
        if [ -f "$SCRIPT_PATH" ]; then
            ...
            (cd "$MULTINODE_DIR" && RAYON_NUM_THREADS=$RAYON_T ./run_radix_multi_ssh.sh "$COMM_T" "$SHIFT" "$HOSTS") 2>&1 | tee >(grep --line-buffered "INFO Total" >> "$LOG_FILE")
            if [ $? -eq 0 ]; then
