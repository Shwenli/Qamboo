...
BIN_PATH="smc_run ./target/release/q12"
...
# Build q12 binary and copy to remote hosts
RUSTFLAGS="-C target-cpu=native" cargo build --release --package experiments --bin q12 --features tcp
scp target/release/q12 $HOST1:$PROJECT_PATH/target/release/
scp target/release/q12 $HOST2:$PROJECT_PATH/target/release/
