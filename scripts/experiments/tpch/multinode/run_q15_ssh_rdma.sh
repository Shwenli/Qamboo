...
BIN_PATH="smc_run ./target/release/q15"
...
# Build q15 binary and copy to remote hosts
RUSTFLAGS="-C target-cpu=native" cargo build --release --package experiments --bin q15 --features tcp
scp target/release/q15 $HOST1:$PROJECT_PATH/target/release/
scp target/release/q15 $HOST2:$PROJECT_PATH/target/release/
