...
BIN_PATH="smc_run ./target/release/q17"
...
# Build q17 binary and copy to remote hosts
RUSTFLAGS="-C target-cpu=native" cargo build --release --package experiments --bin q17 --features tcp
scp target/release/q17 $HOST1:$PROJECT_PATH/target/release/
scp target/release/q17 $HOST2:$PROJECT_PATH/target/release/
