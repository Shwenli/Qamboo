...
BIN_PATH="smc_run ./target/release/q18"
...
# Build q18 binary and copy to remote hosts
RUSTFLAGS="-C target-cpu=native" cargo build --release --package experiments --bin q18 --features tcp
scp target/release/q18 $HOST1:$PROJECT_PATH/target/release/
scp target/release/q18 $HOST2:$PROJECT_PATH/target/release/
