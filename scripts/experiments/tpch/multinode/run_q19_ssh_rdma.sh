...
BIN_PATH="smc_run ./target/release/q19"
...
# Build q19 binary and copy to remote hosts
RUSTFLAGS="-C target-cpu=native" cargo build --release --package experiments --bin q19 --features tcp
scp target/release/q19 $HOST1:$PROJECT_PATH/target/release/
scp target/release/q19 $HOST2:$PROJECT_PATH/target/release/
