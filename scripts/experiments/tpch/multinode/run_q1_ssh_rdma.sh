...
BIN_PATH="smc_run ./target/release/q1"
...
# Build q1 binary and copy to remote hosts
RUSTFLAGS="-C target-cpu=native" cargo build --release --package experiments --bin q1 --features tcp
scp target/release/q1 $HOST1:$PROJECT_PATH/target/release/
scp target/release/q1 $HOST2:$PROJECT_PATH/target/release/
