...
BIN_PATH="smc_run ./target/release/q21"
...
# Build q21 binary and copy to remote hosts
RUSTFLAGS="-C target-cpu=native" cargo build --release --package experiments --bin q21 --features tcp
scp target/release/q21 $HOST1:$PROJECT_PATH/target/release/
scp target/release/q21 $HOST2:$PROJECT_PATH/target/release/
