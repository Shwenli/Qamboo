...
BIN_PATH="smc_run ./target/release/q22"
...
# Build q22 binary and copy to remote hosts
RUSTFLAGS="-C target-cpu=native" cargo build --release --package experiments --bin q22 --features tcp
scp target/release/q22 $HOST1:$PROJECT_PATH/target/release/
scp target/release/q22 $HOST2:$PROJECT_PATH/target/release/
