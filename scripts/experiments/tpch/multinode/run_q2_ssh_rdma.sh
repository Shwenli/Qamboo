...
BIN_PATH="smc_run ./target/release/q2"
...
# Build q2 binary and copy to remote hosts
RUSTFLAGS="-C target-cpu=native" cargo build --release --package experiments --bin q2 --features tcp
scp target/release/q2 $HOST1:$PROJECT_PATH/target/release/
scp target/release/q2 $HOST2:$PROJECT_PATH/target/release/
