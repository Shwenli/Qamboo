...
BIN_PATH="smc_run ./target/release/q4"
...
# Build q4 binary and copy to remote hosts
RUSTFLAGS="-C target-cpu=native" cargo build --release --package experiments --bin q4 --features tcp
scp target/release/q4 $HOST1:$PROJECT_PATH/target/release/
scp target/release/q4 $HOST2:$PROJECT_PATH/target/release/
