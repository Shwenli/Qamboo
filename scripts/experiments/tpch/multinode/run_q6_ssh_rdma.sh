...
BIN_PATH="smc_run ./target/release/q6"
...
# Build q6 binary and copy to remote hosts
RUSTFLAGS="-C target-cpu=native" cargo build --release --package experiments --bin q6 --features tcp
scp target/release/q6 $HOST1:$PROJECT_PATH/target/release/
scp target/release/q6 $HOST2:$PROJECT_PATH/target/release/
