...
BIN_PATH="smc_run ./target/release/q7"
...
# Build q7 binary and copy to remote hosts
RUSTFLAGS="-C target-cpu=native" cargo build --release --package experiments --bin q7 --features tcp
scp target/release/q7 $HOST1:$PROJECT_PATH/target/release/
scp target/release/q7 $HOST2:$PROJECT_PATH/target/release/
