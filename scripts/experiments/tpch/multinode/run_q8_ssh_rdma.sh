...
BIN_PATH="smc_run ./target/release/q8"
...
# Build q8 binary and copy to remote hosts
RUSTFLAGS="-C target-cpu=native" cargo build --release --package experiments --bin q8 --features tcp
scp target/release/q8 $HOST1:$PROJECT_PATH/target/release/
scp target/release/q8 $HOST2:$PROJECT_PATH/target/release/
