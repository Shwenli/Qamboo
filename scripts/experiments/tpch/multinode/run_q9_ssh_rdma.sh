...
BIN_PATH="smc_run ./target/release/q9"
...
# Build q9 binary and copy to remote hosts
RUSTFLAGS="-C target-cpu=native" cargo build --release --package experiments --bin q9 --features tcp
scp target/release/q9 $HOST1:$PROJECT_PATH/target/release/
scp target/release/q9 $HOST2:$PROJECT_PATH/target/release/
