mod rep3_ring_table_groupby{
    ...
    fn tcp_filter_public() {
        ...
        let other_values_ring: Vec<RingElement<u64>> = other_values.iter().map(|v| RingElement(*v)).collect();

        // 分享数据给三方
        let value_shares = rep3_ring::share_ring_elements(&values_ring, &mut rng);
        let other_value_shares = rep3_ring::share_ring_elements(&other_values_ring, &mut rng);
        ...
        // 计算期望结果：valid 列应该只在 value > 50 的位置为 1
        let should_valid: Vec<RingElement<u64>> = values.iter()
            .map(|v| if *v == filter_value { RingElement(1u64) } else { RingElement(0u64) })
            .collect();
        ...
        for (party_id, tx, (value_share, other_value_share, valid_share)) in izip!(
            0..3, 
            [tx1, tx2, tx3], 
            izip!(value_shares, other_value_shares, valid_shares)
        ) {
            ...
            let handle = std::thread::spawn(move || {
                ...
                let other_column = ShareColumn::new(other_value_share, DataType::Arithmetic, "other".to_string());
                let valid_column_bool = conversion::a2b_many(&valid_share, &net, &mut state).unwrap();
                let valid_column = ShareColumn::new(valid_column_bool, DataType::Binary, "valid".to_string());
                //let mut valid_column = ShareColumn::new(valid_column_bool, DataType::Binary, "valid".to_string());
                ...
                table.insert_column("valid".to_string(), valid_column);

                let total_start = std::time::Instant::now();
                // 执行 filter_public 操作（2个网络）
                table.filter_public(
                    value_column.get_name(),
                    predicate,
                    &filter_value,
                    &[&net],
                    &mut [&mut state],
        ...
        println!("Filter public test passed!");
    }

    #[test]
    fn tcp_filter_public_2_networks() {
        const VEC_SIZE: usize = 1000000;
        ...
        let parties_main = vec![
            NetworkParty::new(0, "127.0.0.1:8310".parse().unwrap()),
            NetworkParty::new(1, "127.0.0.1:8311".parse().unwrap()),
            NetworkParty::new(2, "127.0.0.1:8312".parse().unwrap()),
        ];
        
        let parties_fork = vec![
            NetworkParty::new(0, "127.0.0.1:9310".parse().unwrap()),
            NetworkParty::new(1, "127.0.0.1:9311".parse().unwrap()),
            NetworkParty::new(2, "127.0.0.1:9312".parse().unwrap()),
        ];
        ...
        for (party_id, tx, (value_share, other_value_share, valid_share)) in izip!(
            ...
            let handle = std::thread::spawn(move || {
                ...
                let nets = [&net0, &net1];
                let mut states = [&mut state0, &mut state1];
        ...
        println!("Filter public (2 networks) test passed!");
    }

    #[test]
    fn tcp_filter_shared() {
        use table::table_operator::Filter;
        use table::predicate::Predicate;
        ...
        let parties_main = vec![
            NetworkParty::new(0, "127.0.0.1:8400".parse().unwrap()),
            NetworkParty::new(1, "127.0.0.1:8401".parse().unwrap()),
            NetworkParty::new(2, "127.0.0.1:8402".parse().unwrap()),
        ];
        
        let parties_fork = vec![
            NetworkParty::new(0, "127.0.0.1:9400".parse().unwrap()),
            NetworkParty::new(1, "127.0.0.1:9401".parse().unwrap()),
            NetworkParty::new(2, "127.0.0.1:9402".parse().unwrap()),
        ];
        ...
        println!("Filter shared test passed!");
    }

    //cargo test --release --package tests --test table -- table_operator_ring::rep3_ring_table_operator::tcp_filter_shared_binary --exact --nocapture
    #[test]
    fn tcp_filter_shared_binary() {
        use table::table_operator::Filter;
        use table::predicate::Predicate;

        const VEC_SIZE: usize = 6000000;
        ...
        // 分享数据给三方
        let lhs_shares = rep3_ring::share_ring_elements_binary(&lhs_ring, &mut rng);
        let rhs_shares = rep3_ring::share_ring_elements_binary(&rhs_ring, &mut rng);
        let other_shares = rep3_ring::share_ring_elements(&other_ring, &mut rng);

        // 创建初始的 valid 列（全为 1，表示所有行都有效）
        let initial_valid: Vec<RingElement<u64>> = vec![RingElement(1u64); VEC_SIZE];
        let valid_shares = rep3_ring::share_ring_elements_binary(&initial_valid, &mut rng);

        let predicate = Predicate::EqualBinary;
        ...
        for (party_id, tx, (lhs_share, rhs_share, other_share, valid_share)) in izip!(
            ...
            let handle = std::thread::spawn(move || {
                ...
                let mut state1 = Rep3State::new(&net1).unwrap();
                // 创建 ShareTable
                let lhs_column = ShareColumn::new(lhs_share, DataType::Binary, "lhs".to_string());
                let rhs_column = ShareColumn::new(rhs_share, DataType::Binary, "rhs".to_string());
                let other_column = ShareColumn::new(other_share, DataType::Arithmetic, "other".to_string());
                let valid_column = ShareColumn::new(valid_share, DataType::Binary, "valid".to_string());
    ...
    fn filter_public_2_net_t(predicate: Predicate)
    {
        const VEC_SIZE: usize = 1000;
        let mut rng = thread_rng();
        ...
        let (predicate_name, port_offset) = match predicate {
            Predicate::Equal => ("Equal", 0),
            Predicate::GreaterThan => ("GreaterThan", 10),
            Predicate::LessThan => ("LessThan", 20),
            Predicate::GreaterOrEqual => ("GreaterOrEqual", 30),
            Predicate::LessOrEqual => ("LessOrEqual", 40),
            Predicate::NotEqual => ("NotEqual", 50),
            Predicate::EqualBinary => ("EqualBinary", 60),
            Predicate::NotEqualBinary => ("NotEqualBinary", 70),
            Predicate::GreaterOrEqualBinary => ("GreaterOrEqualBinary", 80),
            Predicate::LessOrEqualBinary => ("LessOrEqualBinary", 90),
            _ => panic!("Invalid predicate"),

        };

        let filter_value = 32u64;
        
        // 计算期望结果
        let should_valid: Vec<RingElement<u64>> = values.iter()
            .map(|v| {
                let result = match predicate {
                    Predicate::Equal => *v == filter_value,
                    Predicate::GreaterThan => *v > filter_value,
                    Predicate::LessThan => *v < filter_value,
                    Predicate::GreaterOrEqual => *v >= filter_value,
                    Predicate::LessOrEqual => *v <= filter_value,
                    Predicate::NotEqual => *v != filter_value,
                    Predicate::EqualBinary => *v == filter_value,
                    Predicate::NotEqualBinary => *v != filter_value,
                    Predicate::GreaterOrEqualBinary => *v >= filter_value,
                    Predicate::LessOrEqualBinary => *v <= filter_value,
                    _ => panic!("Invalid predicate"),
                };
        ...
        for (party_id, tx, (value_share, other_value_share, valid_share)) in izip!(
            ...
            let handle = std::thread::spawn(move || {
                ...
                let bind_nets = [&net0, &net1];
                let mut bind_states: Vec<&mut Rep3State> = vec![&mut state0, &mut state1];

                let mut netstate_args = NetStateArgs::new(&bind_nets, &mut state0, &mut state1, &mut bind_states);
    ...
    fn filter_public_2_net() {
        //filter_public_2_net_t(Predicate::Equal);
        //filter_public_2_net_t(Predicate::GreaterThan);
        //filter_public_2_net_t(Predicate::LessThan);
        filter_public_2_net_t(Predicate::GreaterOrEqual);
        //filter_public_2_net_t(Predicate::LessOrEqual);//这个没通过
        //filter_public_2_net_t(Predicate::NotEqual);
        //filter_public_2_net_t(Predicate::EqualBinary);
        //filter_public_2_net_t(Predicate::LessOrEqualBinary);
        //filter_public_2_net_t(Predicate::GreaterThanBinary);
        //filter_public_2_net_t(Predicate::GreaterOrEqualBinary);
        //filter_public_2_net_t(Predicate::NotEqualBinary);
